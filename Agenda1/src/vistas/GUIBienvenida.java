...
public class GUIBienvenida {
	...
	private void initialize() {
		...
		comboBoxAnio.setBounds(173, 62, 54, 22);
		for(int anio = 2020; anio <= 2030; anio++) {
			comboBoxAnio.addItem(Integer.toString(anio));
		}
		panel_2.add(comboBoxAnio);
		
		JLabel labelFechaReserva_1 = new JLabel("Fecha");
		labelFechaReserva_1.setBounds(20, 37, 57, 14);
		panel_2.add(labelFechaReserva_1);
		...
		btnBuscarHistorial.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				String consulta = "SELECT fecha, autopatente, descripcion_servicio, precio FROM historial h NATURAL JOIN servicio s WHERE h.servicio_tipo=s.tipo AND h.autopatente = '"+ patente+"'";
	...
	private void mostrarAgendaHoy() {
		String consulta = "SELECT * FROM historial WHERE DATE(fecha) = NOW()";
	...
	private void mostrarAgendaDia() {
		...
		String consulta = "SELECT fecha, autopatente, nombre, apellido, descripcion_servicio FROM historial h "
				+ "			NATURAL JOIN servicio s NATURAL JOIN persona p NATURAL JOIN auto a "
				+ "			WHERE autopatente = a.patente AND a.persona_dni = p.dni AND"
				+ "			h.servicio_tipo=s.tipo AND DATE(fecha) = '" + diaReserva+"'";
		mostrarTabla(tableAgenda, consulta);
	}
